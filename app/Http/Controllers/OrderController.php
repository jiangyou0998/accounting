...
//            $start_date = '2022-03-01';
            ...
            for ($i = 0; $i < $showDayNum; $i++) {
                ...
                $countArr[$user->kb_bakery_id][$day] = 0;
                $countArr[$user->kb_kitchen_id][$day] = 0;
                $countArr[$user->kb_waterbar_id][$day] = 0;
            }

            foreach ($kbitems as $item){
//                $day = Carbon::parse($item->deli_date)->isoFormat('MM-DD(dd)');
                $day = $item->deli_date;
                $countArr[$item->user_id][$day] += $item->count;
            }

            for ($i = 0; $i < $showDayNum; $i++) {
                $day = Carbon::parse($start_date)->addDay($i)->toDateString();
                $countArr['kb'][$day] = '';
                $countArr['kb'][$day] .= '包-' . ($countArr[$user->kb_bakery_id][$day] ?? 0);
                $countArr['kb'][$day] .= ' 廚-' . ($countArr[$user->kb_kitchen_id][$day] ?? 0);
                $countArr['kb'][$day] .= ' 水-' . ($countArr[$user->kb_waterbar_id][$day] ?? 0);
            }
