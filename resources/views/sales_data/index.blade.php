...
{{--                    period-時段收入--}}
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            <label for="morningIncome">早市收入</label>
                        ...
                        <div class="col-md-4 mb-3">
                            <label for="noonIncome">午市收入</label>
                            <input type="text" pattern="[0-9]*" class="form-control period-income" id="noon_income" value="{{ $sales_income_detail['22'] ?? '' }}" placeholder="" min="0" autocomplete="off" required>
                        </div>

                        <div class="col-md-4 mb-3">
                            <label for="afternoonTeaIncome">下午茶收入</label>
                            <input type="text" pattern="[0-9]*" class="form-control period-income" id="afternoon_tea_income" value="{{ $sales_income_detail['23'] ?? '' }}" placeholder="" min="0" autocomplete="off" required>
                        </div>

                        <div class="col-md-4 mb-3">
                            <label for="eveningIncome">晚市收入</label>
                            <input type="text" pattern="[0-9]*" class="form-control period-income" id="evening_income" value="{{ $sales_income_detail['24'] ?? '' }}" placeholder="" min="0" autocomplete="off" required>
                        </div>

                        <div class="col-md-4 mb-3">
                            <label for="nightSnackIncome">宵夜收入</label>
                            <input type="text" pattern="[0-9]*" class="form-control period-income" id="night_snack_income" value="{{ $sales_income_detail['25'] ?? '' }}" placeholder="" min="0" autocomplete="off" required>
                        </div>
...
{{--                    payment-支付方式收入--}}
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            <label for="foodpandaIncome">Foodpanda</label>
                            <input type="text" pattern="[0-9]*" class="form-control payment-income" id="foodpanda_income" value="{{ $sales_income_detail['34'] ?? '' }}" placeholder="" min="0" autocomplete="off" required>
                        </div>
...
{{--                    payment-支付方式收入--}}
                    <hr>
                    <h4 class="mb-3">4.現金</h4>
                    <div class="alert alert-danger pos-cash-alert" role="alert" style="display: none;"></div>
                    <div class="row">

                        <div class="col-md-6 mb-3">
                            <label for="cash">今日餘數</label>
                            <input type="text" pattern="[0-9]*" class="form-control pos-cash" id="cash" value="{{ $sales_income_detail['41'] ?? '' }}" placeholder="" min="0" autocomplete="off" required>
                        </div>

                        <div class="col-md-6 mb-3">
                            <label for="escortCash">押運公司</label>
                            <input type="text" pattern="[0-9]*" class="form-control pos-cash" id="escort_cash" value="{{ $sales_income_detail['42'] ?? '' }}" placeholder="" min="0" autocomplete="off" required>
                        </div>


                    </div>

                    <h3 class="col-lg-12 p-3 text-right">現金總計 : $<span id="pos_cash_sum">0.00</span></h3>

                    <hr>
                    <h4 class="mb-3">5.昨日餘數</h4>
                    <div class="row">

                        <h3 class="col-lg-12 p-3 text-right">昨日餘數 : $<span id="last_balance">{{$last_balance}}</span></h3>
{{--                        <h3 class="col-lg-12 p-3 text-right">夾萬承上結存 : $<span id="last_safe_balance">{{$last_safe_balance}}</span></h3>--}}

                    </div>

                    <hr>
                    <h4 class="mb-3">6.其他</h4>
                    <div class="alert alert-danger pos-cash-alert" role="alert" style="display: none;"></div>
                    <div class="row">

                        <div class="col-md-6 mb-3">
                            <label for="breadIncome">麵包營業額</label>
                            <input type="text" pattern="[0-9]*" class="form-control pos-cash" id="bread_income" value="{{ $sales_income_detail['61'] ?? '' }}" placeholder="" min="0" autocomplete="off" required>
                        </div>
...
@section('script')
    <script>
        ...
        function getPeriodIncomeSum(){
            ...
            let noon_income = $('#noon_income').val();
            let afternoon_tea_income = $('#afternoon_tea_income').val();
            let evening_income = $('#evening_income').val();
            let night_snack_income = $('#night_snack_income').val();
            let period_sum = 0;
            ...
            if((noon_income.length > 0)){
                period_sum += parseFloat(noon_income);
            }

            if((afternoon_tea_income.length > 0)){
                period_sum += parseFloat(afternoon_tea_income);
            }
            ...
            if((night_snack_income.length > 0)){
                period_sum += parseFloat(night_snack_income);
            }
        ...
        function getPaymentIncomeSum(){
            ...
            let foodpanda_income = $('#foodpanda_income').val();
            let payment_sum = 0;
            ...
            if((foodpanda_income.length > 0)){
                payment_sum += parseFloat(foodpanda_income);
            }
        ...
        function getPosCashSum(){
            let cash = $('#cash').val();
            let escort_cash = $('#escort_cash').val();

            let pos_cash_sum = 0;

            if((cash.length > 0)){
                pos_cash_sum += parseFloat(cash);
            }

            if((escort_cash.length > 0)){
                pos_cash_sum += parseFloat(escort_cash);
            }
        ...
        function getIncomeSum(){
            ...
            let income_sum = 0;
            income_sum += parseFloat(pos_income_sum);
        ...
        function getDifference(){
            ...
            let income_sum = getIncomeSum();
            //收銀機收入
            let payment_income_sum = getPaymentIncomeSum();

            let pos_cash_sum = getPosCashSum();

            let difference = 0;
            difference -= parseFloat(income_sum);
            difference += parseFloat(payment_income_sum);
            difference += parseFloat(pos_cash_sum);
        ...
        $('.btn-submit').click(function () {
            ...
            req.inputs.push({
                ...
                noon_income: $('#noon_income').val(),
                afternoon_tea_income: $('#afternoon_tea_income').val(),
                evening_income: $('#evening_income').val(),
                night_snack_income: $('#night_snack_income').val(),
                octopus_income: $('#octopus_income').val(),
                alipay_income: $('#alipay_income').val(),
                wechatpay_income: $('#wechatpay_income').val(),
                foodpanda_income: $('#foodpanda_income').val(),
                cash: $('#cash').val(),
                escort_cash: $('#escort_cash').val(),
                bread_income: $('#bread_income').val(),
            });
